...
def within_range(tree, i, radius):
    ...
    res = tree.query_ball_point(point_loc, radius)
    for j in res:
        d = distance(i, 2 * j)
...
def IDWv2(i, k=2):
    ...
    for j in res:
        m += 1 / distance(i, 2 * j)
        n += extracted_contents[j][2] / distance(i, 2 * j)
    return n / m if m != 0 else contents[i - 1][2]
...
def LLR(i):
    ...
    for item in in_range:
        points.append(extracted_contents[item[0]])
...
# LLR method
start = time.time()
with open('f6.txt', 'w') as f6:
    for i in range(len(contents)):
        if i % 2:
            cur = contents[i]
            z = LLR(i)
            f6.write(f'{cur[0]}\t{cur[1]}\t{z}\n')
        else:
            f6.write('\t'.join(map(str, contents[i])) + '\n')
end = time.time()
print(f'{"LLR":<10}{end-start+t0:>10.2f}')
